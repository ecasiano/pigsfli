...
namespace pimc
{
    ...
    // ============================================================
    // Boundary kink pair insertion at tau = beta (partner in bulk)
    // ============================================================
    class BoundaryKinkPairInsertionBetaMC : public Move
    {
    public:
        explicit BoundaryKinkPairInsertionBetaMC(const SimulationParameters &params)
            : beta_(params.beta()) {}
        ...
        bool attempt(Configuration &C, RNG &rng) override
        {
            ...
            // 2. times: tau1 = beta, tau2 uniform in (0,beta)
            double tau1 = beta_;
            double tau2 = rng.uniform() * beta_;
            if (tau2 <= 0.0 || tau2 >= beta_)
                return false;

            // 3. occupations at tau=beta
            int n_iB = wl.occupationAt(i, beta_);
            int n_jB = wl.occupationAt(j, beta_);
            if (n_iB <= 0)
                return false;
            ...
            double me1 = abs_t * std::sqrt((double)n_iB * (double)(n_jB + 1));
            double me2 = abs_t * std::sqrt((double)(n_jB + 1) * (double)n_iB);
            if (me1 == 0.0 || me2 == 0.0)
                return false;
            ...
            // 5. insert kinks: i->j at beta, j->i at tau2
            Kink k1{tau1, 0, i, j, -1, -1, r, r, -1, i};
            Kink k2{tau1, 0, i, j, -1, -1, r, r, -1, j};
            auto [idx1, idx2] = wl.insertHop(k1, k2);
            ...
            // 7. proposal ratio
            auto boundary_pairs_new = enumerateBoundaryPairs0(wl, beta_); // reuse same logic
            int N_pairs_new = (int)boundary_pairs_new.size();
            if (N_pairs_new == 0)
            {
                wl.deleteHop(idx1);
    ...
    // ============================================================
    // Boundary kink pair removal at tau = beta (partner in bulk)
    // ============================================================
    class BoundaryKinkPairRemovalBetaMC : public Move
    {
    public:
        explicit BoundaryKinkPairRemovalBetaMC(const SimulationParameters &params)
            : beta_(params.beta()) {}
